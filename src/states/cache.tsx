...
function fetchFiles(files: { name: string; type: string }[]) {
    return Promise.all(
        files.map((file) => {
            ...
            return Promise.all([
                fetch(`https://storage.googleapis.com/dkg-0_4_9-caches/${file.name}.header`).then((res) => res.text()),
                fetch(`https://storage.googleapis.com/dkg-0_4_9-caches/${file.name}`).then((res) => res.text()),
            ]).then(([header, data]) => ({
                file,
                header,
                data,
